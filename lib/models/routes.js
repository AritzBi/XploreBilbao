...
getMyRoutes: function(id, cb){
  pg.connect(conString, function(err, client, done){
    var query="SELECT R.* FROM ROUTE R where r.createdby=$1";
    client.query(query, [id], function(err,result){
      done();
      if(err){
        cb(err,null);
      }else{
        var routes=result.rows;
        query="SELECT R.* FROM ROUTE R, USER_ROUTE U where  u.user_id=$1 and u.route_id=r.id";
        client.query(query, [id],function(err,result){
          done();
          if(err){
            cb(err,null);
          }else{
            routes=routes.concat(result.rows);
            cb(null,routes);
          }
        });
      }
