...
module.exports = {
	findById: function(id, cb){
		pg.connect(conString, function(err,client,done){
			query="SELECT L.ADDRESS, L.GEOM, T.TYPE_DENOM_ES, T.TYPE_DENOM_EN, T.TYPE_DENOM_EU, B.* FROM EMBLEMATIC_BUILDING B, BUILDING_TYPE T, LOCATION L WHERE H.ID=$1 AND L.ID=H.LOCATION_ID AND T.ID=B.BUILDING_TYPE ";
