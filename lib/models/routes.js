...
createRoute: function(values,userId, cb){
    pg.connect(conString,function(err, client, done){
      var query="INSERT INTO ROUTE (name_es, name_en, name_eu, description_es, description_en, description_eu, ROUTE) VALUES ($1,$2,$3,$4,$5,$6,$7) returning ID";
      client.query(query,[values.name,values.name,values.name,values.description,values.description,values.description,values.route], function(err, result){
        if(err){
          console.log(err);
          done();
          cb(err,null);
          ...
          query="INSERT INTO USER_ROUTE (USER_ID,CREATED_AT, CREATEDBY, ROUTE_ID) VALUES ($1,$2,$3,$4)";
          client.query(query,[userId,new Date(),true,result.rows[0].id], function(err,result){
            if(err){
              done();
              cb(err, null);
            }else{
...
getMyRoutes: function(id, cb){
  pg.connect(conString, function(err, client, done){
    var query="SELECT R.*, U.CREATEDBY, U.CREATED_AT FROM ROUTE R, USER_ROUTE U WHERE R.ID=U.ROUTE_ID AND U.USER_ID=$1";
    client.query(query, [id], function(err,result){
      done();
      if(err){
        cb(err,null);
