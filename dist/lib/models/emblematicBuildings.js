...
module.exports = {
	...
	findAll: function(cb){
		pg.connect(config.bdPath, function(err,client,done){
			query="SELECT L.ADDRESS, L.GEOM, T.TYPE_DENOM_ES, T.TYPE_DENOM_EN, T.TYPE_DENOM_EU, B.*,(SELECT AVG(C.NOTE) FROM BUILDING_COMMENTS C, EMBLEMATIC_BUILDING J WHERE J.ID=C.BUILDING_ID AND J.ID=B.ID) as note FROM EMBLEMATIC_BUILDING B, BUILDING_TYPE T, LOCATION L where L.ID=B.LOCATION_ID  AND T.ID=B.BUILDING_TYPE AND T.TYPE_DENOM_ES!='Museo'";
	...
	findByIdGeoJson: function(id, cb){
		pg.connect(config.bdPath, function(err,client, done){
			var query="SELECT row_to_json(f) \
				...
				FROM location as l \
				INNER JOIN (SELECT b.*, T.TYPE_DENOM_ES, T.TYPE_DENOM_EN, T.TYPE_DENOM_EU from EMBLEMATIC_BUILDING b, location l,BUILDING_TYPE T WHERE b.id=$1 and b.location_id=l.id and T.ID=B.BUILDING_TYPE) as lp ON lp.location_id=l.id)as f";
			client.query(query, [id], function(err,result){
				done();
				if(err){
					cb(err,null);
