...
module.exports = {
	...
	findByIdGeoJsonWithoutDetails: function(id, cb){
		pg.connect(conString, function(err,client, done){
			var query="SELECT row_to_json(f) \
				...
				, row_to_json(lp) as properties \
				FROM location as l \
				INNER JOIN (SELECT b.denom_es, b.denom_en, b.denom_eu, b.description_es, b.description_en, b.description_eu,b.location_id t.* \
	...
	findMuseums: function(cb){
		pg.connect(conString, function(err,client,done){
			query="SELECT L.ADDRESS, L.GEOM, T.TYPE_DENOM_ES, T.TYPE_DENOM_EN, T.TYPE_DENOM_EU, B.* FROM EMBLEMATIC_BUILDING B, BUILDING_TYPE T, LOCATION L where L.ID=B.LOCATION_ID  AND T.ID=B.BUILDING_TYPE AND B.BUILDING_TYPE='Museo'";
			client.query(query, [], function(err,result){
				if(err){
					done();
					cb(err, null);
